...
Each record also includes monotonic `time_seconds` for elapsed-time observations. Set a call's `cooperative_cancellation` to `false` to simulate a blocking external operation that cannot process cancellation during its delay; its late completion must not be treated as a confirmed host result. The scenario's optional `startup_delay_seconds` delays protocol startup to exercise initialization timeout and cancellation. Each newly started process reads the scenario file anew and starts at request index zero.
