...
from app.services.events import EventWriter, stored_event_from_row
from app.services.read_models import TestRunDetail, TestRunReport
...
class SqlAlchemyExecutionRepository:
    ...
    async def get_detail(self, test_run_id: str) -> TestRunDetail:
        async with self.sessions() as session:
    ...
    async def get_report(self, test_run_id: str) -> TestRunReport:
        detail = await self.get_detail(test_run_id)
        events = await self.list_events(test_run_id)
        artifacts = await self.list_artifacts(test_run_id)
    ...
    async def load_snapshot(self, test_run_id: str) -> TestRunSnapshot:
        async with self.sessions() as session:
            row = await session.get(TestRunRow, test_run_id)
        if row is None:
            raise LookupError("Test run not found")
        return load_test_run_snapshot(row.snapshot_json)
